...
export default function DashboardPage() {
  const { user, signOut } = useAuth(); // We still need user details and signOut
  const router = useRouter();

  return (
    // Wrap the entire page content (except the layout itself) with AuthCheck
    <AuthCheck
       fallback={ // Provide a more structured loading skeleton
         <AppLayout>
     ...
     >
       {/* The actual dashboard content, rendered only if authenticated */}
       <AppLayout>
         <div className="container mx-auto p-4 md:p-8">
           <div className="flex justify-between items-center mb-8">
             <h1 className="text-3xl font-bold text-foreground">
               {/* Use optional chaining as user is guaranteed by AuthCheck */}
               Welcome, {user?.displayName || user?.email || 'User'}!
             </h1>
              {/* Sign out button remains, handled by useAuth */}
